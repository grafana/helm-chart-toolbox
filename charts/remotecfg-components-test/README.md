...
`/api/v0/web/remotecfg/components` API, and verifies the running components are a superset of the expected set for that
collector `role`. The expected sets are supplied as `EXPECTED_<ROLE>` environment variables (the role, upper-cased) —
...
A check fails if the collector's pods are missing any expected component, or if the expected set for a role is empty
(which means Fleet Management served nothing for that role). The test pod retries `attempts` times with `delay` seconds
between runs.
...
| checks | list | `[]` | Per-collector checks: a list of `{ role, namespace, selector }`. `role` selects the expected component set from the `EXPECTED_<ROLE>` env var (see envFrom); `selector` is the label selector for that collector's Alloy pods. |
